...
exclude_list = ['null_domain', 'root', 'metagenomes', 'environmental sample', 'fuzzy_archaea', 'fuzzy_viruses',
                ...
                'fuzzy_root', 'fuzzy_cellular_organisms', 'fuzzy_unclassified_sequences', 'unclassified_sequences',
                'Viruses']
...
for domain in set(data_mean.domain_name):
    if exclude_list.count(domain) == 0:
        ...
        for year in years:
            year_ids = list(meta_data.loc[(meta_data.year == year)].sra_id)
            year_data = domain_data[domain_data['sra_id'].isin(year_ids)]
            deseq_file = str('../data/DESeq/ETSP_COG_SSU/output/meta/gene_by_domain/' +
                             domain + '/DESeq2_' + str(int(year)) + '_oxic_vs_suboxic.res')
        ...
        plt.title(domain + ' AWPD by depth and year for COGs in ETSP OMZ')
