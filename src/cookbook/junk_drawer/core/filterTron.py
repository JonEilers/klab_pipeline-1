...
for line in input:
	...
	if split_line[0] == 'gene':
		columns = [split_line[0],split_line[1],split_line[2],split_line[5],'normalized_taxa_count',split_line[3],split_line[4],'high_level_classification','sub_high_level_classification','metabolism','read_count_library','Normalization_factor','functional_category','functional_description']
	...
	if split_line[0] != 'gene':
		...
		if out_dictionary.has_key(domain_name) == True:
			out_dictionary[domain_name].write('\t'.join([gene,site,size,taxa_count,norm_tcount,scientific_name,domain_name,high_taxa,sub_high_taxa,metabo,read_count,norm_factor,func_cat,func]) + '\n')
input.close()
output0.close()
output1.close()
output2.close()
