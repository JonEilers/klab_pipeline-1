...
EVALUE=1e-5
PDCUTOFF=1e-2
TAXDB=/home/rkodner/prep/noodle/data/ncbi_taxonomy.db
...
if [ -s $RECRUITED.sto ]; then
  ...
  # Pruning
  /home/rkodner/bin/FastTree -quiet $AUTO1.dedup.fasta > $AUTO1.fast.tre
  rppr prunetre --never-prune-regex-from $EUKTAXIDREX --cutoff $PDCUTOFF \
  ...
  guppy ref_tree -o {title}-reftree.xml {title}.refpkg
fi
